...
export const verifyChallenge = async (req, res) => {
    ...
    try {
        ...
        if (verified && registrationInfo) {
            ...
            const { rows } = await pool.query(`INSERT INTO credential_schema.credentials 
            ...
            sub.connect().then(async () => {
                ...
                await sub.subscribe("dkg-result", async (message) => {
                    try {
                        ...
                        if (parsed.result_type === "dkg-result" &&
                            parsed.id === dkgPayload.id) {
                            console.log(`🧩 Received public key from server ${parsed.server_id}: ${parsed.data}`);
        ...
        await pool.query(`INSERT INTO key_schema.keys (sessionId, userId, solanaAddress, created_at)
       VALUES ($1, $2, $3, NOW());`, [sessionId, userId, sharedPubkey]);
...
//# sourceMappingURL=verifyChallengeController.js.map